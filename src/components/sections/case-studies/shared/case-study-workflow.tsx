      ...
      <div className="relative z-10 mx-auto max-w-[1200px] text-white">
        <div className="flex flex-col items-stretch gap-3 lg:flex-row lg:gap-1.5">
          {data.steps.map((step, index) => (
            <div key={step.title} className="flex flex-1 items-stretch gap-1.5">
              <article className="flex h-full w-full flex-col justify-center rounded-2xl border border-white/10 bg-white/[0.06] px-3 py-4 text-center backdrop-blur-md sm:px-4 sm:py-5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--color-electricBlue)]">
                  {step.title}
                </p>
              ...
              {index < data.steps.length - 1 ? (
                <span
                  className="hidden shrink-0 self-center text-2xl font-semibold leading-none text-[var(--color-electricBlue)] lg:inline"
                  aria-hidden
                >
                  ›
                </span>
        ...
        <div className="mt-10 grid grid-cols-1 gap-8 text-center md:mt-12 md:grid-cols-3 md:gap-6">
          {data.highlights.map((item) => (
            <div key={item.title}>
              <p className="text-sm font-semibold uppercase tracking-[0.1em] text-[var(--color-electricBlue)]">
                {item.title}
              </p>
