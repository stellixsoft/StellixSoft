...
import { renderBlogContentHtml } from "@/src/lib/blog-content";
...
import { blogPostingJsonLd } from "@/src/lib/schema";
import { buildArticleMetadata } from "@/src/lib/seo-metadata";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const ogImageUrl = coverSrc.startsWith("http") ? coverSrc : absoluteUrl(coverSrc);
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const coverSrc = getBlogCoverImageSrc(post);
  const coverAbsolute = coverSrc.startsWith("http") ? coverSrc : absoluteUrl(coverSrc);
  const html = renderBlogContentHtml(post.content, post.contentFormat);
  ...
  const jsonLd = blogPostingJsonLd({
    ...
    tags: post.tags,
    coverImageAbsoluteUrl: coverAbsolute,
  });
    ...
    <>
      ...
      <article className="px-4 py-12 sm:px-6 md:py-16">
        <div className="mx-auto grid max-w-[1100px] grid-cols-1 gap-10 lg:grid-cols-[7fr_3fr] lg:gap-12 lg:items-start">
          ...
          <BlogPostSidebar currentSlug={post.slug} posts={allPosts} />
        </div>
      </article>
